...
const renderProducts = () => {
    ...
    // Abrufen aus dem lokalen Speicher
    let cart = JSON.parse(localStorage.getItem("cart")) || { items: [] }; //Initialisieren eines leeren Einkaufswagens, wenn sich nichts im LocalStorage befindet
...
let slideIndex = [1,1];  /* ein Array mit zwei Werten, die aktuelle slide anzeigt*/
let slideId = ["mySlides1", "mySlides2"] /* ein Array mit den Klassennamen der HTML-Elemente, die slides von slideshow enthält */

/* ruft dann die Funktion showSlides() zweimal mit den Argumenten 1 und 0 sowie 1 und 1 auf, um erste slide von slideshow initialisiert*/
showSlides(1, 0);
showSlides(1, 1);
        
function plusSlides(n, no) { /* n die Anzahl der slides und no der Index der slide show ist*/
...
function showSlides(n, no) {
    ...
    /* ob n größer als die Anzahl der slides ist. */
    if (n > x.length)
        {slideIndex[no] = 1} /* setzt slideIndex bei Index Nr. auf 1 */

    if (n < 1)
    {slideIndex[no] = x.length} /* setzt slideIndex bei Indexnummer auf die Gesamtzahl der slideshow */

    for (i = 0; i < x.length; i++) {
        x[i].style.display = "none";  /* durchläuft alle slides und setzt display property auf "none" */
    }
    x[slideIndex[no]-1].style.display = "block";   /*  der slide am aktuellen Index auf block*/
}
